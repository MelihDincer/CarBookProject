...
namespace CarBookProject.Persistence.Repositories.StatisticsRepositories
{
    public class StatisticsRepository : IStatisticsRepository
    {
        ...
        {
            int pricingId = Convert.ToInt32(_context.Pricings.Where(x => x.Name == "Günlük").Select(y => y.PricingID));
        ...
        {
            int pricingId = Convert.ToInt32(_context.Pricings.Where(x => x.Name == "Aylık").Select(y => y.PricingID));
            return await _context.CarPricings.Where(x => x.PricingID == pricingId).AverageAsync(y => y.Amount);
        }

        public async Task<decimal> GetAvgRentPriceWeeklyAsync()
        {
            int pricingId = Convert.ToInt32(_context.Pricings.Where(x => x.Name == "Haftalık").Select(y => y.PricingID));
            return await _context.CarPricings.Where(x => x.PricingID == pricingId).AverageAsync(y => y.Amount);
        }
